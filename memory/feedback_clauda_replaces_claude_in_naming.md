...
description: "Clauda" (and "Claudette") replace "Claude" in all Martinez Methods naming — thread personas, AI assistant labels, internal role names — to avoid trademark/IP infringement concerns with Anthropic's "Claude" mark. Applies going forward.
type: feedback
...
1. **Thread persona names and AI assistant labels in Martinez Methods work use Clauda or Claudette.** Examples as of 2026-04-24:
   - **Clauda the Experiment PI** (D2R Methodology Factorial work)
   - **Claudette the Failure Fixer** (plugin expansion, ASAE adaptation to more MAST failure modes)
   - **Clauda the Value Genius** (IP + market value work)
   - **Claudette the Code Debugger** (app-build debugging)
2. **When referring to Anthropic's product, "Claude" is still correct** (Claude Code, Claude Opus 4.7, the Claude API, etc.) — these are factual references to Anthropic's trademarked product, not Martinez Methods branding.
3. **Existing product names that incorporate "Claude"** (Claude Cost, Claude Clarified Chat) are legacy and carry IP risk. Any new Martinez Methods product name must use Clauda. Rename-to-Clauda for existing products is Krystal's call; do not unilaterally rename.
4. **In commit messages, documentation, and memory files**, the rule applies to persona/brand references only, not to references to Anthropic's product or model family.
5. **When in doubt**, default to Clauda/Claudette for Martinez Methods artifacts; reserve "Claude" exclusively for references to Anthropic's actual product.
...
- `feedback_ip_discipline_filesystem.md` — filesystem-level IP
