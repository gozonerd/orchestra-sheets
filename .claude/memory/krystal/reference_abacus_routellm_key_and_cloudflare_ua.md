...
- **Today / active (OneDrive):** `~/OneDrive/.api-keys/super-resume-verification.env` — the _same_ path in Git-Bash notation is `/c/Users/NerdyKrystal/OneDrive/.api-keys/super-resume-verification.env` (one location, two notations — not two files).
