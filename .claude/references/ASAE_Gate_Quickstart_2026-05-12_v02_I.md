...
**MANDATORY before authoring any verification brief or running any rater:** read `references/Independent_Verification_Brief_Best_Practices_2026-05-18_v01_I.md`. Supreme law: **raters are INPUT; the parent adjudicates them exactly as it would input from a sib Claude.** No "remove/weaken X because it seems implausible/inflated" finding is ever actionable as stated — it converts to a deterministic test; absent a deterministic refutation the claimant's evidence STANDS (burden on the exclusion claim). Briefs must test the artifact's truth, not a prescribed surface pattern.
